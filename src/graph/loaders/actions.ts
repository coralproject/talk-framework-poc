...
const genActionsByItemID = (_, item_ids) => {
  return ActionsService
    ...
    .then(arrayJoinBy(item_ids, 'item_id'));
};
 ...
 */
const genActionSummariessByItemID = (ctx, item_ids) => {
  return ActionsService
    .getActionSummaries(item_ids, ctx.user.id)
    .then(arrayJoinBy(item_ids, 'item_id'));
};
 ...
 */
const getItemIdsByActionTypeAndItemType = (ctx, actionType, itemType) => {
  return ActionModel.distinct('item_id', {
